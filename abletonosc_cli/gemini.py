"""Gemini audio-critique client for the AbletonOSC composer.

Sends rendered audio to Gemini's native audio-understanding models so the agent
can "hear" and critique its own work. Verified facts (see TipsNTricks.md):

- Endpoint: POST /v1beta/models/{model}:generateContent, key in x-goog-api-key.
- Audio is sent inline as base64 ``inline_data`` (fine for clip/section WAVs,
  which are well under the 20 MB inline limit); ~32 audio tokens/sec.
...
# act on (drive fx / device set / note edits) rather than prose.
SYSTEM_PROMPT = (
    "You are a meticulous music producer and mix engineer reviewing a short "
    "excerpt of a work-in-progress track produced in Ableton Live. Listen to the "
    "AUDIO and give concrete, actionable feedback an automated agent can apply. "
    "Be specific about frequencies, instruments, timing, and arrangement. If you "
    "are unsure about exact pitch/tuning, say so rather than guessing.\n\n"
    ...
    '  "arrangement_notes": ["..."],\n'
    '  "tuning_timing": ["..."],\n'
    '  "suggestions": [{"change": "...", "how": "<e.g. fx eq8 cut low-mids, '
    'lower track 2 volume, quantize hats>"}]\n'
    "}\n"
...
class GeminiError(Exception):
    """Raised for config/transport/parse problems (mapped to exit 2)."""
...
def _load_dotenv_key() -> Optional[str]:
    ...
    candidates = [
        ...
        os.path.join(os.path.dirname(here), ".env"),  # repo root next to package
    ]
